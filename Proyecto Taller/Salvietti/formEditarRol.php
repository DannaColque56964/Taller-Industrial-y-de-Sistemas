...
<body>
    ...
    $id = $_GET['id'];
    $resultado = $conn->prepare("SELECT * FROM Rol WHERE IdRol = ?");
    $resultado->execute([$id]);
    $roles = $resultado->fetchAll(PDO::FETCH_ASSOC);
    ?>
    <h1>Categorias</h1>
    <form action="javascript:editarRol()" method="POST" id="formEditarRol">
        <?php foreach ($roles as $rol) { ?>
            <div class="row mb-3">
            <input type="hidden" name="Id" value="<?php echo $id; ?>">
                <label for="descripcion" class="col-sm-1 col-form-label">Descripcion:</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" id="descripcion" name="descripcion" value="<?php echo $rol['DescripcionRol'] ?>">
                </div>
